...
- LOG_FILE_PATH (required, default for Docker '/var/www/html/nginx.log') - path to the log file. For docker usage should be also mounted as volume
- GIT_SSH_URL (required) - ssh repo url
- SSH_KEY_PATH (required, default for the Docker '/var/www/html/id_rsa') - path to the ssh key for repo connection. For docker usage should be also mounted as volume
...
- GIT_EMAIL (optional, default 'service@test.com') - git email
- GIT_BRANCH (optional, default 'dev') - git branch
...
docker run \
  ...
  -v /var/log/nginx/nginx.log:/var/www/html/nginx.log \
  my-image \
  --sort datetime=DESC bytes_sent=ASC \
  --filter 'datetime>26/Apr/2021:21:20:22 +0000' 'request_id%45' \
  --message 'Test message' \
